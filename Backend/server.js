  ...
  jwt.verify(token, SECRET_KEY, async (err, user) => {
    if (err) {
      ...
      if (err.name === 'TokenExpiredError') {
        // Send session expiry email asynchronously
        if (user && user.email) {
          sendSessionExpiryEmail(user.email, 'Your Device').catch(e => console.error('Failed to send expiry email:', e));
        }
        return res.status(403).json({ error: "Session expired - please login again", expired: true });
      }
